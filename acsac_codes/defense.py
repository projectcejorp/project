...
from defense_utils import SpectrogramMask, is_overlap, win2block, mask_param, mask_param_auto,mask_param_sv2, generate_mask_params, get_one_text
...
def mask_search(args,
    ...
    sp_mask = None
    tgt_mel = mel_load(tgt_path)
    base_mel = copy.deepcopy(tgt_mel)
    emb_spk, emb_ori, emb_infer, delta_infer = mask_param(tgt_path, src_path, out_path, es_path, encoder)
...
def mask_search_auto(args,
    ...
    if args.model == 'chou':
        emb_spk, emb_ori, emb_infer, delta_infer = mask_param(tgt_path, src_path, out_path, es_path, encoder)
    if args.model == 'auto':
        emb_spk, emb_ori, emb_infer,delta_infer = mask_param_auto(tgt_path, src_path, out_path, es_path, encoder)

    print('check the raw infer', delta_infer)
    ...
    for epoch in range(args.b_num):
        ...
        for i in range(args.b_num):
            ...
            for mask in ['Zero', 'AN', 'GB']:
                ...
                if args.model == 'auto':
                    repeat = 5
                    tgt_mask, mask_sr = torchaudio.load(mask_path)
                    mask_mel = wav2mel(tgt_mask, mask_sr).to('cuda')
                ...
                for r in range(repeat):
                    ...
                    if args.model == 'auto':
                        autovc_infer(src_path, mask_mel, out_infer)

                    wav_infer = preprocess_wav(Path(out_infer))
                    emb_infer = encoder.embed_utterance(wav_infer)

                    # Calculate quality change of synthetic speeches
                    d_QI = delta_infer - np.inner(emb_ori, emb_infer)
                    delta_QI_list.append(d_QI)
...
def mask_search_sv2(args,
    ...
    if args.model == 'auto':
        emb_spk, emb_ori, emb_infer,delta_infer = mask_param_auto(tgt_path, src_path, out_path, es_path, encoder)
    if args.model == 'sv2tts':
        text = get_one_text()
        emb_spk, emb_ori, emb_infer, delta_infer = mask_param_sv2(tgt_path, text, out_path, es_path, encoder)
        ...
        for i in range(args.b_num):
            ...
            for mask in ['Zero', 'AN', 'GB']:
                ...
                for r in range(repeat):
                    ...
                    if args.model == 'sv2tts':
                        sv2_infer(text, mask_path, out_infer)
