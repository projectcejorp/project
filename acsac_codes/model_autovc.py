...
def get_distance(v1,v2):
  ...
  emb_v1 = encoder.embed_utterance(preprocess_wav(Path(v1)))
  emb_v2 = encoder.embed_utterance(preprocess_wav(Path(v2)))
  dis = np.inner(emb_v1,emb_v2)
  return dis,emb_v1,emb_v2

def tmel2nmel(mel):
  return mel.T.data.cpu().numpy()

def plot_mel(mel):
  plt.figure(figsize=(25,10))
  librosa.display.specshow(mel, x_axis="s", y_axis="mel",sr=24000,hop_length=300 )
  plt.colorbar(format = "%+2.f")

def plot_mel_vc(mel):
  plt.figure(figsize=(25,10))
  librosa.display.specshow(mel, x_axis="s", y_axis="mel",sr=16000,hop_length=200, fmin=50 )
  plt.colorbar(format = "%+2.f")
