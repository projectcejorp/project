...
from defense_utils_v2 import emb_simi, get_one_text
from defense_v2 import mask_search_test
from inference_v2 import chous_infer, autovc_infer, sv2_infer
...
def main():
    ...
    if args.system == 'chou':
        ...
        autovc_model = torch.jit.load('auto_vc_model/model.pt').cuda()
        autovc_vocoder = torch.jit.load('auto_vc_model/vocoder.pt').cuda()
        wav2mel = Wav2Mel()
    ...
    try:
        for idx, tgt_wav in enumerate(tgt_files):
            ...
            print(f'Check the quality of modified synthetic speech: before: {QI_before:.3f}, after: {QI_after:.3f}')
